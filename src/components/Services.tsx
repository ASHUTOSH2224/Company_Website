        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={index}
                className={`service-card group cursor-pointer transition-all duration-700 delay-${index * 100} ${
              ...
              >
                {/* Service Icon */}
                <div className={`w-12 h-12 bg-gradient-to-br ${service.color} rounded-lg flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300`}>
                  <Icon size={24} className="text-white" />
                </div>

                {/* Service Content */}
                <div className="space-y-4">
                  <h3 className="heading-sm text-white group-hover:text-[#0070f3] transition-colors duration-300">
                    {service.title}
                  </h3>
                  ...
                  <div className={`space-y-2 transition-all duration-300 ${
                    isHovered ? 'opacity-100 max-h-32' : 'opacity-60 max-h-20'
                  }`}>
                    {service.features.map((feature, featureIndex) => (
                      <div key={featureIndex} className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bg-[#0070f3] rounded-full"></div>
                        <span className="text-body-sm text-xs">{feature}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className={`pt-4 transition-all duration-300 ${
                    ...
                    <button 
                      ...
                      className="text-[#0070f3] text-sm font-medium flex items-center gap-2 hover:gap-3 transition-all duration-300"
                    >
                      Learn More
                      <ArrowRight size={14} />
                    </button>
